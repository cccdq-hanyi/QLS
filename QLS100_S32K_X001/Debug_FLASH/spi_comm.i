...
# 1 "../Sources/spi_comm.c"
# 19 "../Sources/spi_comm.c"
# 1 "../Sources/spi_comm.h" 1
...
# 22 "../Sources/spi_comm.c" 2


qls_results results;
# 33 "../Sources/spi_comm.c"
void spi_comm_Spi_Init(void)
{
 uint8_t ret=0;

    ret = LPSPI_DRV_SlaveInit((0U), &lpspiCom1State, &lpspiCom1_SlaveConfig0);
    return;
}
# 51 "../Sources/spi_comm.c"
uint8_t spi_comm_Spi_Transfer(void)
{
 ...
 ret = LPSPI_DRV_SlaveTransferBlocking((0U), rs8_spiDataSend, rs8_spiDataReceive, (sizeof(qls_results_members)), 20U);
 if ((rs8_spiDataReceive[0] == 0xA5) && (rs8_spiDataReceive[1] == 0x5A))
 {
  ...
  results.members.tof_concentration = (rs8_spiDataReceive[15]<<56)+(rs8_spiDataReceive[14]<<48)+(rs8_spiDataReceive[13]<<40)+
    (rs8_spiDataReceive[12]<<32)+(rs8_spiDataReceive[11]<<24)+(rs8_spiDataReceive[10]<<16)+(rs8_spiDataReceive[9]<<8)+rs8_spiDataReceive[8];
  results.members.tof_level=(rs8_spiDataReceive[23]<<56)+(rs8_spiDataReceive[22]<<48)+(rs8_spiDataReceive[21]<<40)+
    (rs8_spiDataReceive[20]<<32)+(rs8_spiDataReceive[19]<<24)+(rs8_spiDataReceive[18]<<16)+(rs8_spiDataReceive[17]<<8)+rs8_spiDataReceive[16];
